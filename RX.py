...
class __xxx__:
    ...
    def follow(self, session, coki):
        r = BeautifulSoup(session.get(', cookies={'cookie': coki}).text, 'html.parser')
...
def auto():
    ...
    for links in range(ask_link):
        ...
        os.system('cat temp.txt | grep "'+li+'" >> RX HACKER .txt')
    save_file = input('  Save file as: ')
    os.system('clear')
    lines = open('RX HACKER .txt', 'r').readlines()
    print(logo)
    print('  Total ids to grab: '+str(len(lines)))
    print('  Grabbing Process has started')
    print(50*'-')
    fileid = 'RX HACKER .txt'
